...
def training_routine(name, net, train_loader, val_loader, epochs, criterion, optimizer, scheduler, logging):
    ...
    for epoch in range(epochs):
        ...
        # train the network
        for batch_n, (batch_data, batch_labels) in enumerate(train_loader)
            
            if batch_n % 100 == 0:
                print('\rTraining epoch {:4} Batch {:6} ({:.2%})'.format(epoch + 1, batch_n, batch_n / len(train_loader)), end=' ' * 10)
            ...
            train_accuracy += batch_correct / len(train_loader.dataset)
        ...
        for batch_n, (batch_data, batch_labels) in enumerate(val_loader)
            ...
            # track overall epoch performance
            val_accuracy += batch_correct / len(val_loader.dataset)
